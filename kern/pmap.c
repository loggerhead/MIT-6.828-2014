...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref > 0 || pp->pp_link != NULL) {
		panic("page_free failed: maybe page is already freed or never alloced");
		return;
	}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	// The relevant page
		...
		if (pp != NULL) {
			pp->pp_ref++;
			*ppde = PTE_ADDR(page2pa(pp)) | PTE_U | PTE_W | PTE_P;
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for (offset = 0; offset < size; offset += PGSIZE) {
		pte_t *ppte = pgdir_walk(pgdir, (const void *) (va + offset), 1);
		if (ppte == NULL) {
			panic("boot_map_region failed: pgdir_walk return NULL");
			return;
		} else {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	uintptr_t begin = ROUNDDOWN(vanum, PGSIZE);
	uintptr_t end = ROUNDUP(vanum + len, PGSIZE);
	perm |= PTE_P;

	for (; begin < end; begin += PGSIZE) {
		pte_t *ppte = pgdir_walk(env->env_pgdir, (void *) begin, 0);
		// if permission not correct or address above ULIM
		if (ppte == NULL || (*ppte & perm) != perm || begin > ULIM) {
